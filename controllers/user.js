...
export const updatecharaList = async (req, res) => {
  try {
    // 檢查傳入的商品 ID 格式
    if (!validator.isMongoId(req.body.charasheetsixs)) throw new Error('ID')
    // 檢查購物車內有沒有商品
    ...
    if (idx > -1) {
      ...
      // 沒有商品，檢查商品是否存在
      const product = await Charasheetsix.findById(req.body.product).orFail(new Error('NOT FOUND'))
      // 商品沒有上架，錯誤
